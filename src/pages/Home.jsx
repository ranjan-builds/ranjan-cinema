...
const Home = () => {
  ...
  const { moviesData, loading, hasFetched, isInitialized, refreshMovies } =
    useMovieContext();
  ...
  const hasMovies = categoryConfigs.some(
    (category) => moviesData[category.key]?.length > 0
  );

  console.log(
    "Home Render - allLoaded:",
    allLoaded,
    "hasMovies:",
    hasMovies,
    "isInitialized:",
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="text-center mb-12">
            ...
            <p className="text-xl text-gray-400 max-w-2xl mx-auto">
              Discover, explore, and save your favorite movies across all genres
              and languages
            </p>
          </div>
                  ...
                  <div className="bg-gray-800/50 rounded-2xl p-4 text-center border border-gray-700/50">
                    <div className="text-2xl font-bold text-yellow-400 mb-1">
                      {categoryConfigs.reduce(
                        (total, cat) =>
                          total + (moviesData[cat.key]?.length || 0),
                        0
                      )}
                      +
                    </div>
                    ...
                    <div className="text-2xl font-bold text-yellow-400 mb-1">
                      {
                        categoryConfigs.filter(
                          (cat) => moviesData[cat.key]?.length > 0
                        ).length
                      }
                    </div>
                      ...
                      <>
                        ...
                        <div className="flex items-center justify-between mb-8">
                          <div className="flex items-center gap-4">
                            <div
                              className={`w-12 h-12 bg-gradient-to-br ${category.gradient} rounded-2xl flex items-center justify-center text-white shadow-lg`}
                            >
                              {category.icon}
                            </div>
                            <div>
                        ...
                        {/* Movies Content */}
                        <div
                          className={`bg-gradient-to-br ${category.bgGradient} rounded-3xl p-6 border border-gray-700/50 backdrop-blur-sm`}
                        >
                          {categoryLoading ? (
                            <LoadingSkeleton />
              ...
              {!hasMovies && allLoaded && (
                <div className="text-center py-20 bg-gray-800/30 rounded-3xl border border-gray-700/50">
                  ...
                  <p className="text-gray-400 text-lg mb-8 max-w-md mx-auto">
                    We couldn't load any movies. This might be a temporary
                    issue.
                  </p>
                  <button
